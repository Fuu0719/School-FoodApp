...
- 門市開始／結束時間使用 24 小時制滾輪，API 驗證 HH:mm；結束早於開始顯示翌日，相同時間拒絕。時段目前為門市展示資料，下單仍依營業日而非精確時段判斷。
...
- 新註冊會員與商家密碼限制為 8–16 字元；既有較長密碼仍可登入，避免舊帳號被鎖住。商品數字欄以淺色 0 提示，新建時不用先刪除實體 0。
...
「膳解人意」以個人化餐飲推薦與即期食品減廢為目標，已具備 Flutter、Node.js API 與 MySQL 持久化。學校已有後端部署；目前手機測試使用筆電 MySQL/API 與 ngrok HTTPS，學校尚未部署 004 及本頁手機測試更新。
...
MySQL 8.0 應設為 Windows 自動服務，但每次重開機後仍先確認資料庫與結構。必須同時保持兩個 PowerShell 視窗：一個執行 API，另一個執行 ngrok。
...
筆電需保持開機、聯網且不進入睡眠；關閉 API 或 ngrok 任一視窗，手機就無法使用雲端功能。目前筆電 API/ngrok 不是 Windows 自動服務，重開機後必須人工啟動。
...
| 專案 | C:\School\my_app；最後由使用者畫面確認版本 8ab08cf |
...
| 遷移 | 001、002、003 已驗收；004 及後續手機測試功能尚未部署學校 |
| 筆電最新驗證 | 2026-09-24：後端 44 通過／3 跳過，真實 MySQL 22 通過，Flutter 112 通過／2 跳過，analyze 無問題 |
...
2026-09-24 筆電驗證：Flutter 完整測試 112 通過／2 跳過，`flutter analyze` 無問題；後端測試 44 通過／3 真實 MySQL 跳過，另以筆電 MySQL 8.0 執行 22 項整合測試全數通過。APK `1.0.0+7` 已建置並安裝 Samsung 實機，透過 ngrok HTTPS 顯示筆電 MySQL 商品；實機已確認會員摘要列、商品目錄與底部安全區。
